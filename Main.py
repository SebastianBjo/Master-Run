...
class Enemy:
    def __init__(self, wave, is_boss=False, ranged=False):
        ...
        self.radius = 20 if not is_boss else 40
        self.speed = 1 + wave*0.1
        self.max_health = 30 + wave*5 if not is_boss else 500 + wave*20
        self.health = self.max_health
    ...
    def update(self, player, enemies):
        ...
        for other in enemies:
            if other != self:
                ...
                if dist_to_other < self.radius*2:
                    move_x += (self.x - other.x) / dist_to_other * 0.2
                    move_y += (self.y - other.y) / dist_to_other * 0.2
        ...
        if self.ranged and time.time() - self.last_shot > self.shoot_cooldown:
            ...
            self.projectiles.append(Projectile(self.x, self.y, velx, vely, 5, 0))
            self.last_shot = time.time()
