...
class Weapon:
    ...
    def upgrade(self):
        ...
        self.splash += 1 if self.splash > 0 else 0
...
class Player:
    ...
    def shoot(self, mouse_buttons, mouse_pos):
        if mouse_buttons[0] and time.time() - self.last_shot > self.shot_cooldown:
            ...
            # Shotgun multi-pellets
            if self.weapon and self.weapon.name=="Shotgun":
                for spread in [-0.2, -0.1, 0, 0.1, 0.2]:
                    velx = math.cos(angle+spread)*8
                    vely = math.sin(angle+spread)*8
            ...
            if self.weapon: self.shot_cooldown = self.weapon.fire_rate
    ...
    def trade_xp_for_hp(self):
        if self.xp>=100:
            self.xp-=100
            self.health=min(self.max_health,self.health+0.1*self.max_health)
...
class Projectile:
    def __init__(self,x,y,velx,vely,damage,splash):
        ...
        self.radius=5
        self.color=ORANGE if splash>0 else GREEN
        self.damage=damage
        self.splash=splash
    def update(self):
        self.x+=self.velx
...
weapons=[
    ...
    Weapon("Shotgun",8,0.6,0,"Spread",25),
...
def open_stat_shop(player):
    ...
    while shop_open:
        ...
        items=[
            ...
            f"3.FireRate(-0.05s) - Cost:{10*player.level}",
            f"4.SP(+1) - Cost:{10*player.level}",
            f"5.Splash(+1) - Cost:{10*player.level}",
            f"6.Trade 100XP for 10% HP",
            "ESC.Exit"
        ]
        ...
        for event in pygame.event.get():
            ...
            if event.type==pygame.KEYDOWN:
                ...
                elif event.key==pygame.K_6: player.trade_xp_for_hp()
...
def open_weapon_shop(player):
    ...
    while shop_open:
        ...
        for i,w in enumerate(weapons):
            ...
            WIN.blit(font.render(f"{w.name}-Cost:{w.cost}-Lvl:{w.level}",True,WHITE),(x+10,y+10))
